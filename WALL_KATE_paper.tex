...
In this paper we will consider fractional differential equations (FDEs) of the form 
...
Many numerical solvers for FDEs discretize the domain on a uniform mesh; the uniform mesh results in a linear system with a Toeplitz matrix. Still FDEs present unique difficulties in numerical methods due to the non-locality of the operator resulting in a dense stiffness matrix and the end-point singularities which make Galerkin finite element methods low-order \cite{hierarch,superfast}.  Jin and Zhou developed a reconstruction technique that isolates the singularity to improve FEM convergence \cite{JZ}. Additionally, superfast preconditioners can be used to solve the system with the conjugate gradient squared method with a cost of $\bigOh(n \log n)$ per iteration \cite{superfast}. 

These methods, however, require uniform meshes. An optimal mesh would have the $L^{\infty}$ error equally distributed on each interval. In general a uniform mesh does not satisfy this condition, and indeed the endpoint singularities of FDEs forces higher error in the intervals near the boundary. Thus to achieve desired accuracy near the end-point singularities the uniform mesh is in general much finer than necessary in other areas of the domain, resulting in unnecessary computations. One alternative is to use an adaptive mesh and adaptive finite element method (AFEM). Adaptive mesh refinement allows the mesh to be refined, e.g. bisected, on certain intervals where an a posteriori error estimate is high. This approach gets us closer to equi-distributed error and thus reduces the $L^{\infty}$ on the whole domain while using less intervals than the uniform mesh would require \cite{AFEM, Xiaozhe}. The usual AFEM process iterates through the process 
...
Our aim is to build a preconditioner for the stiffness matrix, $\A$ in AFEM that will reduce this work. We will investigate what structure the adaptive mesh retains and how that is reflected in the structure of $\A$. Then using preconditioning techniques formulated for Topelitz matrices \cite{Chan_89, Chan_Strang, Chan_Yeung, schrod}, we will build our own preconditioner.  We prove the preconditioned system has eigenvalues clustered around 1 and demonstrate numerical results with superlinear convergence. 
...
There has long been interest in solving Toeplitz linear systems efficiently. A matrix $A$ is called Toeplitz if $a_{ij} = a_{i-j}$. Arbitrary $n \times n$ matrices have up to $n^2$ entries and are in general, solved directly in $\bigOh(n^3)$ time. Since a Toeplitz matrix has at most $2n-1$ unique entries, we may expect to be able to solve it in $\bigOh (n^2)$ time. This is indeed the case via techniques such as Levinson's algorithm \cite{Levinson}. Even this improvement, however, is infeasible for large systems. Instead we turn to iterative Krylov and multigrid methods \cite{schrod, hierarch, Xiaozhe}. For these methods we can still take advantage of Toeplitz structure by using circulant preconditioners. 

Toeplitz matrices commonly arise in partial differential equations (PDE) discretization, FDEs, signal processing, and control theory. Often the Toeplitz matrices are also symmetric positive definite (SPD). Given an SPD Toeplitz system $\Toe x = b$, the idea introduced by Strang and Chan is to use certain circulant preconditioners $C$ so that $C^{-1}Tx = C^{-1}b$ is solved in fewer iterations \cite{Chan_Strang}.
...
As discussed in section \ref{stiffness}, the stiffness matrix resulting from AFEM is not Toeplitz, but does contain SPD Toeplitz blocks. 
To build an effective preconditioner and investigate its properties, we have to take full advantage of these blocks. Toeplitz matrices have many unique properties, for our purposes we focus on their connection to functions in the Wiener class. This will allow us to take our problem from matrix operator theory to function theory. Suppose we have a singly infinite, symmetric Toeplitz matrix
\begin{equation*}
	\Toe = \begin{pmatrix}
		\toe_0 & \toe_1 & \toe_2 & \\
...
\begin{equation} \label{Hank}
	...
	\Hank = \begin{pmatrix}
		...
		\vdots & \vdots & \vdots & s\cdots & \vdots & \vdots & \vdots \\
		0 & 0 & 0 & \cdots & \toe_5 & \toe_4 & \toe_3 \\
		0 & 0 & \toe_{n-1} & \cdots & \toe_4 & \toe_3 & \toe_2
	\end{pmatrix}.
\end{equation}
Then Bini and Di Benedetto's preconditioner is defined as $\ptau := \Toe - H$. $\Hank$ is a Hankel matrix, that is it is constant on the antidiagonals. Both $\Toe$ and $\Hank$ are symmetric, so they can be completely represented by the at most $n$ unique values: the diagonals of $\Toe: \toe_0, \toe_1, \dots, \toe_{n-1}$, and the antidiagonals of $\Hank: \toe_2, \toe_3, \dots, \toe_{n-1},0,0 $. 
	
The following lemma summarizes properties of $\ptau$.
\begin{lemma}[Properties of $\ptau$, see \cite{Bini}]
...
We will now establish the structure and properties of the stiffness matrix $\A$ resulting from AFEM on equation \ref{FDE}. During the $\texttt{MARK}$ module of AFEM \ref{AFEM}, it is often the case that adjacent elements have similar error estimates, and are either both marked or both not marked for refinement. As a result, after several refinements there will still be subsections of the domain that have been refined to the same level. We refer to these subdomains as \emph{locally uniform}.  Since uniform meshes give rise to SPD Toeplitz systems, if we formed the stiffness matrix for a locally uniform subdomain we would have an SPD Toeplitz matrix. So wherever there are adjacent elements of the same size we can find a corresponding Toeplitz block on the main diagonal of our stiffness matrix, $\A$. The size of this block depends on how many adjacent elements are the same size. In practice we have observed that the boundary of the domain almost always requires the greatest level of refinement. In general we have larger Toeplitz blocks from locally uniform subdomains in the middle of the matrix, and smaller blocks---or indeed $1 \times 1$ blocks---near the boundary. We use $\A_d$ to denote the block diagonal with Toeplitz blocks part of $\A$. 

As mentioned previously, the fractional derivative results in a dense matrix. The $ij$ entry of $\A$ reflects the strength of the interaction between elements $i$ and $j$. Thus we observe off-diagonal decay, as elements that are physically separated in the domain have weaker interaction. The rate of decay depends on the fractional order $\alpha$. The low rank representation makes for fast computations, but complicates both the implementation of the preconditioner and the spectral clustering of the preconditioned system.  Blocks that are most distant from the diagonal contain less information and we assume they have small-norm. We denote these blocks as $\A_F$. Blocks that are adjacent to the diagonal are not in general small-norm, but due to the weaker interaction, are well-suited for low rank approximation. (See \cite{Xiaozhe} for more details.)  These blocks are denoted $\offd$. These assumptions are made precise in section \ref{proof} and numerically validated in section \ref{validate}. Using this notation we can write our stiffness matrix as the sum of the Toeplitz blocks, blocks adjacent to the diagonal, and blocks further removed from the diagonal, $\A = \A_D + \offd + \A_F$. 
...
In this section we set forth the properties we require from a preconditioner, how we use $\ptau$ in building our preconditioner $\Tau$, and how to build and apply our preconditioner efficiently.
...
\begin{enumerate}
	\item Complexity of constructing and applying $\Tau$ should be $\bigOh (m \log m)$.
	\item A linear system with $\Tau$ should be solved in $ \bigOh (m \log m)$ operations.
	\item The spectrum of $\Tau^{-1} \A$ should be clustered around 1
\end{enumerate}
...
\end{align*}
\todo[inline]{SPSD justifying 2-norm to sing val to eig}
...
\end{figure}
In figure \ref{fig:eigen_clusters} we can see some example results of the preconditioner on the spectrum of $\A$. These tests are for the problem in $\ref{FDE}$ with $\alpha = 1.7$ and exact solution $u(x) = 10x^2 (1-x^2)$ on $[0,1]$ (see \cite{Xiaozhe} example 6.1 for more details). The three different matrix sizes are $n = 31, 71, 2795$ and reflect 0, 3, and 11 levels of refinement respectively. Note that zero levels of refinement corresponds to a uniform mesh. 
