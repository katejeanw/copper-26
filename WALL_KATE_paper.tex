...
\begin{abstract}
	...
	Preconditioning on adaptive grid, theoretical and numerical results for eigenvalue clustering 
\end{abstract}
...
\section{Introduction} \cite{KoMa14}
...
\begin{itemize}
	...
	\item Toeplitz matrices $\iff$ generating functions 
	\item Truncated infinite matrix
	\item Hankel 
...
\begin{definition}[Eigenvalue Clustering] 
	Note $\A_m$ has real eigenvalues. For any $\varepsilon > 0$ we say the eigenvalues of a matrix $\ptau^{-1} A$ are clustered around $1$ if there exists $N_1$ and $N_2$ such that for all $m > N_1$ there are at most $N_2$ eigenvalues of $\ptau^{-1}A_m$ that do not lie within $ [1-\varepsilon, 1+\varepsilon]$.
\end{definition}
...
\subsubsection{Notes on gap between above and my case}
\begin{itemize}
	\item Can be simplified with $B = \zero$
	\item Bounding $a(z)$ away from zero, numerically? Complex differentiation?
\end{itemize}


Actual condition: $a(z) > 2 \varepsilon$, is this true??
...
\begin{itemize}
	\item Weyl's inequality vs min-max characterization -- are they saying the same thing?
	\item for one-off blocks we can't write eigs as some point on $a(z)$, does this break things? 
	\item Note on choosing $m$ vs choosing $\varepsilon$
	\item numerical test confirming off-diag low rank
	\item Explanation and tests showing off-off-diag are small norm 
	\item For notation, do we like $\Tau$ as big $\tau$? 
