...
\begin{itemize}
	...
	\item $\ptau$ is diagonalized by the type-I discrete sine transform matrix, $S$. We write $\ptau = S \Lambda S^{-1} = S \Lambda S$.  
...
This explicit construction is not the most efficient, instead we can apply $\Tau$ implicitly in $m \log m $ time where $m = \sum_{j=1}^k m_j$. 
